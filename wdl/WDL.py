...
class WDL():
    ...
    def fit(self, X: torch.Tensor,
        ...
        :param dictionaryUpdate: specifies how the dictionary should be updated
        :param dictOptimizer: the torch optimizer to use if not updating the dictionary jointly
        :param dictOptKWargs: the keyword arguments to the torch optimizer for the dictionary
        ...
        :param weight_init: how the weights should be initialized
        ...
        :param support: the fixed support of the distributions (not needed for regular grid data like images)
        ...
        :param jointOptimizer: the torch optimizer to use for jointly optimizing the weights and dictionary
        :param jointOptimKWargs: the keyword arguments to the joint torch optimizer
        ...
        :param height: used only if the data is on a regular grid like an image - the number of bins in the height
        :param width: used only if the data is on a regular grid like an image - the number of bins in the width
        ...
        for restart in range(n_restarts):
            ...
            if init_method == "rand":
                ...
                self.D = dictClass(wassersteinKMeansInit(X, k=self.n_atoms, OTmethod=OTsolver))
                ...
                for iter in range(max_iters):
                    ...
                    if iter % log_iters == 0:
                        ...
                        if log:
                            ...
                            curr_idx = int(iter / log_iters)
                            log_dict["loss"][curr_idx] = loss.detach()
    ...
    def updateVariables(self):
        """
        updates variables in joint optimizer or just the dictionary since the coding step handles the weights
        :return:
        """
        if self.joint:
    ...
    # computes the objective function of GeoWWDL
    def computeLoss(self):
        ...
        for i in range(self.n):
            p = self.barycenterSolver(D, weights[:, i])

            # compute loss
            loss += self.OTsolver(self.X[:, i], p)
        ...
        if self.mu != 0.0:
            for i in range(self.n):
                ...
                intermediate_loss += weights[:, i].dot(self.OTsolver(self.X[:, i], D))

                # add each data's loss to the overall loss
                loss += self.mu * intermediate_loss
    ...
    # function to verify that a result of some distance computation as resulted in nan
    def validateLoss(self, loss):
        if torch.isnan(loss):
            warnings.warn(f"loss become nan on iteration {iter}, exiting update loop", RuntimeWarning)
            if iter == 0:
...
def histRegression(D: torch.Tensor, p: torch.Tensor, baryMethod, otMethod,
                   ...
                   maxIter: int = 100, verbose: bool = False, tol=1e-6, device: torch.device = torch.device("cpu")):
    """
...
class GenericVariable:
    ...
    # only used when not being jointly optimized
...
class ExpCovHist(GenericVariable):
    def __init__(self, variable: torch.Tensor, joint: bool, optimizer: type = None, optKWargs={}) -> None:
        ...
        super().__init__(inverseCOV(variable).detach().clone(), joint, optimizer, optKWargs=optKWargs)
