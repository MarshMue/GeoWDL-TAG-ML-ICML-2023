...
### The code in this file is centered around solving the quadratic program described in theorem 1 of the paper
# "Measure Estimation in the Barycentric Coding Model" bt Werenski et al.
# found here: https://proceedings.mlr.press/v162/werenski22a/werenski22a.pdf
# getBCMWeights is the primary function used by our code
...
def getBCMweights(D: torch.Tensor, x: torch.Tensor, embeddings: torch.Tensor, reg: float,
    ...
    Given a dictionary D and data x, compute the wasserstein barycentric weights to represent the data using the
    dictionary atoms by solving the quadratic program in the BCM paper

    :param D: a (d x m) tensor of the m atoms (probability distributions as columns)
    :param x: a (d x n) tensor of the n data distributions

    :return weights: a (m x n) tensor of the barycentric weights for each of the n data distributions
    :return return_vals: an optional tensor with the n objective values of the QP
    """
...
def solve(inner_products, return_val=False):
    ...
    ### Setup and solve the quadratic program to determine barycentric weights
    # (theorem 1) https://proceedings.mlr.press/v162/werenski22a/werenski22a.pdf
